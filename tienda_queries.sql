...
SELECT nombre, precio AS precio_euros, precio*1.08 AS precio_dolar from producto;
...
-- 10.lista els noms i els preus de tots els productes de la taula producto, 
...
SELECT DISTINCT(codigo_fabricante) AS codigo FROM producto;
...
SELECT producto.nombre AS nombre_producto, precio, fabricante.nombre AS nombre_fabricante FROM producto
...
SELECT p.codigo AS id_prod, p.nombre AS producto, f.codigo AS id_fabricante, f.nombre AS fabricante FROM producto p
JOIN fabricante f ON p.codigo_fabricante = f.codigo;
...
JOIN producto ON fabricante.codigo = codigo_fabricante
...
SELECT fabricante.nombre AS fabricante, p.nombre AS producto FROM fabricante
LEFT JOIN producto p ON fabricante.codigo = codigo_fabricante;
...
JOIN fabricante f ON f.codigo = codigo_fabricante
...
AND precio > (
SELECT SUM(precio)/COUNT(precio) FROM producto p 
JOIN fabricante f ON p.codigo_fabricante = f.codigo
WHERE f.nombre = "ASUS");